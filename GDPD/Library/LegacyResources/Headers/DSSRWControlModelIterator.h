...
//    virtual int CountNonNullRows();
    ...
    // weilin US114129 4/1/18 Calendar Selector Empty Check
    bool isCalendarSelectorEmpty();

    // weilin US126676 8/1/18 Check valid selection
    bool isValidSelection(unsigned Int32 absolutePos);
    bool isValidDate(int iYear, int iMonth, int iDay);
    int getStartAndEndDate(MDataType::DSSDateTime &startDate, MDataType::DSSDateTime &endDate, int &dateType);
private:
    ...
    int hGetElementDisplayText(int iKey, string& orText);
    
    //generate recursive unit level information
    int hGenerateLevelInfo();
    int hClearCacheValues();

    // weilin 8/1/18 check whether it is a calendar selector
    bool hIsCalendarSelector();
    int hGetDateElementData(int iIndex, const void **oppData, int &oDataType);
private:
    ...
    vector<int> mvElementsWithoutSort;
    ...
    MDataType::DSSDateTime* mpStartDate;
    MDataType::DSSDateTime* mpEndDate;
    EnumDSSDataType mDateType;
    bool mbStartEndFetched;
};
