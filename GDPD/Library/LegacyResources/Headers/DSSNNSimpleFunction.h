...
public:
    ...
    virtual HRESULT Calculate(int nParam, CComVariant *ipParams, CComVariant *opResult, DSSDataFlag *opFlag) = 0;
	
	int CalculateRepeat(int nParam, int nRepeat, CComVariant *ipParams, CComVariant *opResult, DSSDataFlag *opFlag);
    
    virtual HRESULT GetOutputType (int size, EnumDSSDataType *inputType, EnumDSSDataType *outputType) = 0;
