...
typedef struct FilterExpressionLeaf
{
	...
	EnumDSSDataType * mpTypes = NULL;
	Int32 * mpcbDatas = NULL;
	unsigned char ** mppDatas = NULL;
	DSSDataFlag * mpFlags = NULL;
    
    FilterExpressionLeaf(const FilterExpressionLeaf &leaf){
        copyToThis(leaf);
    }
    FilterExpressionLeaf& operator=(const FilterExpressionLeaf &leaf){
        freeMemory();
        copyToThis(leaf);
        return *this;
    }
...
private:
    void copyToThis(const FilterExpressionLeaf &leaf);
    void freeMemory();
} FilterExpressionLeaf;
