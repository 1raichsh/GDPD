...
public:
    ...
    virtual HRESULT createIterator(DSSRWSectionModelIterator *ipParent, DSSRWGridIterator **opIterator);
    ...
    HRESULT getDataModel(DSSRWDataModel **ppDataModel);
    
    int GetXTabView(DSSXTabView **ppXTabView);
    
    virtual DSSXTabView * &getXTabView();
    
    HRESULT CreateXTabView(DSSXTabView **ppXTabView);
    
    HRESULT GetActualGBElements(DSSRWSectionModelIterator* ipParentIterator, vector<vector<int> >& ovvActualGBElements, int& oLocalTag);
    ...
    HRESULT getRunTimeUnits(vector<int> & ovRunTimeUnits);
    ...
    // 5/24/2012 tehe
    virtual HRESULT EnsureModelData(bool *obUpdated = NULL);
    int UnLoadSliceData();
...
public:
    ...
    HRESULT getDataNullDisplay(DSSDataFlag iNullFlag, MBase::CString &orVal);
    
    HRESULT getMaxDataOffset(int iMetricIndex, bool ibSkipNull = false);
    
	HRESULT getMinDataOffset(int iMetricIndex, bool ibSkipNull = false);
...
private:
    ...
    vector<vector<DSSMaskInfos*> > mvpMetricHideMasks;
