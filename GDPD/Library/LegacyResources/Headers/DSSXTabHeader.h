...
private:
    ...
    int hComputeHide2OrgMap(std::vector<bool *>& ivMetricRowNull, Int32 inMetric);
    
    //DE120497
    vector<vector<DSSMaskInfos*> >* getMetricHideMaskForRow();
    
    //the input index should be index from original index (including hidden rows)
    //this funcion is called in initBySelection and initMetricGroupby, the init() should have been invoked before that.
    inline void SetBitForNonNegativeIndex(DSSXTabBitVector& iBitVector, Int32 iIndex)
    {
        if (iIndex < mvTargetPos.size())
            iIndex = mvTargetPos[iIndex];
        
        if (iIndex >= 0)
        {
            iBitVector.Set(iIndex, true);
        }
    }
    
    inline void hUpdateRowIndexOriginal(int& iRow) {
        if (mpHide2OrgMap && iRow < mnDispRows)
        {
            iRow = mpHide2OrgMap[iRow];
        }
    }
    ...
    int mnOrgDispRow;
    int* mpHide2OrgMap; //disp row to original row
    vector<int> mvTargetPos; //original row to disp row
...
public:
    inline int getCountRecursiveUnits() { return mnRecursiveUnits; };
    int CalculateUnionMask( DSSTabularIndex* ipTargetIndices, DSSXTabBitVector ** ippCDSSXTabBitVec, Int32 iCurTag,
                           std::vector<DSSXTabBitVector*>* opMetricBitVector, Int32* iPageByMetricIndex);
    
    int getFirstLevelByUnits(vector<int> &irvUnits, int * orSubtotalID);
};
