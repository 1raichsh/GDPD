...
{	
	...
	/*
     DE143853; This helper function to check whether a polygon need to draw border.
     When the border set to NONE, the format line width is = 0.
     */
    inline bool PolygonObject::hIsBorderNeeded() const
    {
        if (mChartContextPtr->IsGraphMatrix()) {
            Int32 lLineWidth = mFormatLinePtr->mLineThickness;
            return lLineWidth > 0;
        }
        return true;
    }
