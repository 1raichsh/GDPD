...
public:
	...
	HRESULT Calculate(int nSize, double * pVec, double * pResult, DSSDataFlag * pFlag );
	...
	HRESULT DirectCalculate(int iStart,
											  int * ipGroupLinkList,
											  int * ipBlockAddress,
...
private:
	...
	HRESULT SetInputParam (int nSize, EnumDSSDataType  dataType, CComVariant *pData);

	int CalculateMaxMin();
	
	HRESULT CalculateCount();
	
	HRESULT CalculateMode();
	
	int CalculateMode(int nSize, double * pVec, double * pResult, DSSDataFlag  * pFlag); // Numerical version
	
	HRESULT Order(int row1, int row2, int& result);
