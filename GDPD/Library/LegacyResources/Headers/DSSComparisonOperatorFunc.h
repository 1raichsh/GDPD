...
public:
    ...
    HRESULT Calculate(Int32 nSize, CComVariant *pVec, CComVariant *opResult, DSSDataFlag *pFlag);
    virtual HRESULT setProperty(Int32 index, CComVariant *pValue);
    //int Init(EnumDSSFunction iFunctionIndex);
    HRESULT GetOutputType(Int32 size, EnumDSSDataType *inputType, EnumDSSDataType *outputType);
    HRESULT Calculate(Int32 nSize, double *pVec, double *pResult, DSSDataFlag *pFlag);
    HRESULT CalculateRepeat(Int32 nParam, Int32 nRepeat, double **pParam, double *pResult, DSSDataFlag *pFlag);
...
private:
    ...
    HRESULT hTryConvert(Int32 iSize, Int32* ipOffset, void *ipData, EnumDSSDataType iType, double iDivider, double **opData);
