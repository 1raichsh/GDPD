...
const int Invalid_Index = -200;
const int Unset_Index = -300;
...
public:
    ...
    /*!
     @method   return whether dataset has a dummy attribute
     */
    bool hasDummyAttribute();
    ...
    int getRowColIndexOfDataCellHandle(DSSCellHandle* iHandle, int* oRow, int* oCol);
	/*!
	 @method
	 @abstract   get the cell handle
	 @discussion
    ...
    int getElementThresholdAt(EnumDSSAxisName iAxis, int iRowNum, const GUID& iUnitID, int &orThresholdID, bool ibTransfer = true);
	/*!
	 @method
	 @abstract   return the number of rows/column the element occupies
	 @discussion iDepth is 0-based. In most cases, iDepth is equal to iUnit, except
    ...
    bool isExpandAll(int iAxis, int iDepth);
    bool isCollapseAll(int iAxis, int iDepth);

    int getGeoAttributeElementCountForMapWidget();

    // weilin DE60560 2017/5/3 oState: SORT_UNDEFINED = 0, SORT_INITIAL = 1, SORT_DESCEND = 2, SORT_ASCEND = 3
    void getDropZoneAxisSortInfo(int &oState, bool &oIsByMetric, int &oSortByMetricIndex, DSSBaseElementsProxy* &oElements);
    
    // weilin DE67958 8/7/17 find if there is consolidation/customgroup in previous unit of the same axis
    bool hasConsolidationOrCustomgroupInPreviousUnit(DSSCellHandle &iCellHandle);
    
    bool IsFROptimized();
    // weilin DE125567 1/15/19
    bool IsOutlineInvalidRow(int iAxis, int iOrdinal);
private:
...
//	int hGetRowColumnOrdinal(DSSCellHandle &irHandle, int &orRow, int &orCol);
    ...
    int hGetUnitThreshold(EnumDSSAxisName iAxis, int iRowNum, int lRowNum, const GUID& iUnitID, int &orThresholdID);
    ...
    // weilin DE60078 2017/5/4 Set display from for cellhandle
    int hSetDisplayForm(DSSCellHandle &orCellHandle);
...
protected:
	...
	int hCreateKeyTable(int nRows, vector<vector<int> > &irOrdinal, vector<vector<int> > &irKeys, vector<int> &orKeyTable, bool ibCreateSubtotalFlag = false);
    ...
    void hPopulateOutlineModeDataStructures();
    bool hTwoLinesSamePrefixOnRow(int first, int second, int depth);
    inline bool IsSubtotalKey(int key) { return key < 0 && key != Invalid_Index;}
    ...
    // weilin US116347 4/25/18
    vector<int> mvUnitDistinctElementsCount[2];
    ...
    vector<int> mvOutlineRowWindow;
    vector<int> mvOutlineRowKeyTable;
    vector<vector<int> > mvOutlineRowEleOrdinals;
    vector<vector<int> > mvOutlineRowEleKeys;
    vector<int> mvCountSubtotalValues;//subtotal values for each column. If multiple subtotals enabled, just count one.
    vector<int> mvFirstSubtotal;
    int mnOutlineRows;
    int mnOutlineDisplayRows;
    DSSXTabBitVector* mpOutlineHiddenRows;
    DSSXTabBitVector* mpOutlineCollapseRows;
    DSSXTabBitVector* mpOutlineSectionRows;
    std::pair<int, int> mOrdinalMappingHelper;// <displayOrdinal, outlineOrdinal> this is a helper info for calcOrdinalConsiderHiddenRows
    bool mbOutlineMode;
    SubTotalMap mSubtotalMap;  //outline row ordinal => depth
...
public:
    ...
    int getFirstMergedElementOrdinal(int iAxis, int iOrdinal, int iDepth);
    int getParentElementText(int iAxis, int iOrdinal, int iDepth, int iBaseFormId, string& oText);
    inline void setInitialized(bool iFlag) { mbIsInitialized = iFlag; }
    //outline mode
    int collapse(int iAxis, int iDepth, int iOrdinal, bool ibAdjustOrdinal = true);
    int expand(int iAxis, int iDepth, int iOrdinal, bool ibAdjustOrdinal = true);
    bool IsCollapseAt(int iAxis, int iDepth, int iOrdinal);
    bool IsFirstCellOfBlock(int iAxis, int iDepth, int iOrdinal);
    bool IsUnitHidden(int iAxis, int iDepth);
    bool IsAllCollapsedOutline(int iAxis, int iDepth);
    int expandAllOutline(int iAxis, int iDepth);
    int collapseAllOutline(int iAxis, int iDepth);
    int calcOrdinalConsiderHiddenRows(int iOrdinal);
    // weilin DE83770 1/10/18 input: iOrdinal(outlineOrdinal), return displayOrdinal
    int calcDisplayOrdinal(int iOrdinal);
    std::string getFirstSubtotalNameOnRow(int iDepth);
    int NextDisplayRowIndex();
    bool StartCheckingHiddenRows();
    inline vector<vector<int> >& RowEleOrdinals() { return mbOutlineMode ? mvOutlineRowEleOrdinals: mvRowEleOrdinals; }
    inline vector<vector<int> >& RowEleKeys() { return mbOutlineMode ? mvOutlineRowEleKeys: mvRowEleKeys; }
    void updateOutlineModeFlag();
    bool isOutlineMode();
    inline void updateOutlineModeDataStructures() { hPopulateOutlineModeDataStructures(); }
    //the following two functions are not used, but keep them
    bool isOutlineSectionRow(int iRow, bool iTransfer = true);
    void getRowSubtotalMap(SubTotalMap& oSubtotalMap);
    int getGlobalAttributeElementCount(int iAxis, int iDepth);
    int getTemplateUnitDistinctElementCount(int iAxis, int iDepth);
    
    bool hasSortDefinedOnCellHandle(DSSCellHandle &iHandle, int iAxis);
    bool isFRGrid();
    
    EnumDSSMetricType getSubtotalTypeFromID(int iId, int iAxis);
    bool hasSubtotalOnAxis(int iAxis);
private:
    // tehe 11/18/2011 support for new rwd
    ...
    //outline mode
    int hGetElementKey(int iAxis, int iDepth, int iOrdinal, bool iTransform = true);
    ...
    // weilin 11/2/17 remove the mpParentIterator which is duplicate to DSSRWIterator::mpParentIterator
...
public:
    ...
    bool getInitialSelectedElementsForGrid(GUID& iAttributeID, vector<DSSBaseElementsProxy*> &oElements);
    bool getMultipleInitialSelectedElements(std::set<GUID> &iAttributeIDs, std::vector<std::vector<DSSBaseElementsProxy*> > &oElements);
    bool getSimplifiedInits(std::set<GUID> &iAttributeIDs, std::set<std::string> &oElements, std::vector<GUID> &iAttrOrderedIDs);
    int getDataColumnCount();
    std::vector<TemplateSelectionRow*>* getSelectionRows();
#endif
};
