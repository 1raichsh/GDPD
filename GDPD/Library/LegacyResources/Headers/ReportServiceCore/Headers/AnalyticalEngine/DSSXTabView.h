...
public:
    ...
    //imihai 06-10-2013
    int getOffsetForCoordinates(int iRow, int iCol, int &lOffset, DSSCubeDataSlice *ipSlice, DSSDataFlag &pFlags);
    
    //zhyang
    int sortForMicroChart(vector<int> &orRows, GUID &iMetricGUID, bool ibAsc = true, bool isTreeMode = false, int iSubtotalID = -100);
	
    // weilin DE70833 9/6/17
    int GenMetricsRowMap(DSSXTabDispInfo *ipDispInfo, MCESchema *ipSchema);
private:
...
public:
    ...
    int getRawCellInfo2(map<int, int> imElementIDs, int iNumber, bool ibRowKey, vector<int> &ivMask, int iMetric,
                        ...
                        DSSDataFlag *opFlags,
                        int &actualUnits,
                        bool ibUseStackTotal = false,
                        int *opDisplayIndexLevel = nullptr
                        );
    ...
    inline void setMetricHideMaskInfo(vector<vector<DSSMaskInfos*> >* ipVMaskInfos) {
        if (ipVMaskInfos && ipVMaskInfos->size() != 0)
            mvpMetricHideMaskInfos = ipVMaskInfos;
    }
    
    inline vector<vector<DSSMaskInfos*> >* getMetricHideMaskInfos() { return mvpMetricHideMaskInfos; }
...
private:
    ...
    vector<vector<DSSMaskInfos*> >* mvpMetricHideMaskInfos;
    //zhyang, extreme
    ...
    // weilin 9/6/17
    int mnTempRow;
    map<DSSTabularIndex *, int *> mMetricsRowMap;
    
    // weilin 9/25/18 Refers to mvRowWindow/mvColumnWindow in DSSRWGridIterator
    vector<int> *mvpDispRows[2];
private:
