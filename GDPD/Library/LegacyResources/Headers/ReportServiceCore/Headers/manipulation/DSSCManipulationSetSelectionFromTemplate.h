...
private:
    ...
    //DE116142 special case handling, for selection only target infowindow we don't want use NGBALL logic, because NGB data is not changed and refresh document will cause infowindow not show.
    bool mIsGridOnlyTargetInfowindow;
public:
    ...
    void setIsGridOnlyTargetInfowindow(bool isOnlyTargetInfoWindow);
	int Execute();
...
inline void DSSCManipulationSetSelectionFromTemplate::setIsGridOnlyTargetInfowindow(bool isOnlyTargetInfoWindow) {
    mIsGridOnlyTargetInfowindow = isOnlyTargetInfoWindow;
}
